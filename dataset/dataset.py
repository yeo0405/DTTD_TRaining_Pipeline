...
class DTTDDataset(data.Dataset):
    def __init__(self, root, mode, refine=False, add_noise=True, config_path="/home/yeo/Downloads/POSE/dataset/dataset_config"):
    ...
    def __getitem__(self, index):
        ...
        obj_found = False # flag to check if we found a valid object with enough pixels in the mask
        for obj_idx in obj_indices:
            ...
            if len((mask_label * mask_depth).nonzero()[0]) > self.minimum_px_num:
                obj_found = True
                break
        # if no valid object found, return a negative sample (with zero label and depth)    
        if not obj_found:
    ...
    def _get_negative_sample(self, img, label, depth, meta):
        """return a negative sample with zero label and depth, and a random crop of the image"""
        img_h, img_w = label.shape
        
        # use the whole image if it's smaller than 400x400, otherwise randomly crop a 400x400 region
        rmin, rmax = 0, min(img_h, 400)  # for simplicity, we can also just take the top-left 400x400 region if the image is larger than 400x400, or we can randomly crop. Here we choose to take the top-left region.
        cmin, cmax = 0, min(img_w, 400)
        
        img_crop = np.transpose(np.array(img)[:, :, :3], (2, 0, 1))[:, rmin:rmax, cmin:cmax]
        
        # return negative sample with zero label and depth, and a random crop of the image
        return {
            "img": torch.from_numpy(np.array(img)),
            "label": torch.from_numpy(label),
            "depth": torch.from_numpy(depth.astype(np.float32)),
            "point_cloud": torch.zeros((self.sample_2d_pt_num, 3), dtype=torch.float32),
            "sample_2d": torch.zeros(self.sample_2d_pt_num, dtype=torch.long),
            "img_crop": self.norm(torch.from_numpy(img_crop.astype(np.float32))),
            "sampled_model_pt_camera": torch.zeros((self.get_model_point_num(), 3), dtype=torch.float32),
            "sampled_model_pt": torch.zeros((self.get_model_point_num(), 3), dtype=torch.float32),
            "obj_id": torch.LongTensor([-1]),  # -1 indicates no object
            "R": torch.eye(3, dtype=torch.float32),
            "T": torch.zeros((1, 3), dtype=torch.float32),
            "is_negative_sample": True,  # mark as negative sample
        }
        ...
        if "symmetry" in self.classes.columns:
            for idx, row in self.classes.iterrows():
                if int(row["symmetry"]) == 1:
                    self.symmetry_obj_cls.append(idx - 1)

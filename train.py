...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="device number, not valid when set data parallel",
)
parser.add_argument(
    "--dataset_root",
    type=str,
    default="./dataset/dttd_iphone/DTTD_IPhone_Dataset/root",
    help="dataset root dir (" "YCB_Video_Dataset" ")",
)
parser.add_argument(
    "--dataset_config",
    type=str,
    default="./dataset/dttd_iphone/dataset_config",
    help="dataset config dir",
)

parser.add_argument("--loss", type=str, default="adds", choices=["adds", "abc"])
parser.add_argument(
    "--recon_choice", type=str, default="depth", choices=["depth", "model"]
)
...
parser.add_argument(
    "--base_latent", type=int, default=128, help="base latent dim for unimodal encoder"
)
parser.add_argument(
    "--embed_dim", type=int, default=512, help="embedding dim for transformer encoder"
)
parser.add_argument(
    "--fusion_block_num", type=int, default=1, help="number of fusion block"
)
parser.add_argument(
    "--layer_num_m", type=int, default=3, help="layer num for modality fusion per block"
)
parser.add_argument(
    "--layer_num_p",
    type=int,
    default=3,
    help="layer num for point-to-point fuison per block",
)
...
parser.add_argument(
    ...
    type=str,
    default="./out",
    help="output directory, store output info",
)
parser.add_argument(
    "--optim_batch", type=int, default=4, help="number of batch per optim"
)
parser.add_argument(
...
parser.add_argument(
    "--save_interval", type=int, default=100, help="Save checkpoint every N epochs"
)
...
parser.add_argument(
    "--lr_rate", default=0.3, type=float, help="learning rate decay rate"
)
...
parser.add_argument(
    "--w_rate", default=0.3, type=float, help="balancing rate decay rate"
)
parser.add_argument(
    "--decay_margin", default=0.033, type=float, help="margin to decay lr & w"
)
parser.add_argument(
    "--decay_rate", default=0.77, type=float, help="decay rate for decay margin"
)
parser.add_argument(
    "--noise_trans",
    default=0.03,
...
parser.add_argument(
    "--nepoch", type=int, default=120, help="max number of epochs to train"
)
parser.add_argument(
    "--resume_posenet", type=str, default="", help="resume PoseNet model"
)
parser.add_argument("--start_epoch", type=int, default=0, help="which epoch to start")

parser.add_argument(
    "--single_obj_test", type=int, default=-1, help="train based on a single obj"
)
parser.add_argument(
    "--exclude_single_obj", type=int, default=-1, help="exclude a single obj"
)
parser.add_argument(
    "--trans_w",
    default=0.5,
    type=float,
    help="weight of direct translation loss",
)
...
def cosine_scheduler(
    ...
    if cur_epoch is None:
        schedule = np.array(
            [
                min_lr
                + 0.5 * (lr - min_lr) * (1 + math.cos(math.pi * i / (len(iters))))
                for i in iters
            ]
        )
        ...
        return warmup_schedule[
            cur_epoch * steps_per_epoch : cur_epoch * steps_per_epoch + steps_per_epoch
        ]
    else:
        return np.array(
            [
                ...
                + 0.5
                * (lr - min_lr)
                * (1 + math.cos(math.pi * (i - warmup_iters) / (len(iters))))
                for i in np.arange(
                    cur_epoch * steps_per_epoch,
                    cur_epoch * steps_per_epoch + steps_per_epoch,
                )
            ]
        )
...
def translation_loss_fn(tx, pt_cld, tar_t):
    if tx.dim() == 4:
        tx = tx.squeeze(2)
    if pt_cld.dim() == 4:
        pt_cld = pt_cld.squeeze(2)

    pred_t = pt_cld + tx
    error = pred_t - tar_t.unsqueeze(1)
    distance = torch.norm(error, dim=-1)
    loss = torch.mean(torch.sqrt(error.pow(2) + 1e-6))
...
def main():
    ...
    train_dataset = DTTDDataset(
        root=opt.dataset_root,
        mode="train",
        add_noise=True,
        config_path=opt.dataset_config,
    )
    test_dataset = DTTDDataset(
        root=opt.dataset_root,
        mode="test",
        add_noise=False,
        config_path=opt.dataset_config,
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=1, shuffle=True, num_workers=opt.workers
    )

    print(f"[TRAIN] sym list: {train_dataset.get_sym_list()}")

    # set model
    estimator = PoseNet(
        num_points=train_dataset.get_2d_sample_num(),
    ...
    if opt.pretrain:
        ...
        diff_params = {
            k: v for k, v in tar_params.items() if k not in src_params.keys()
        }
    ...
    adds_loss = ADDS_Loss(
        num_points_mesh=train_dataset.get_model_point_num(),
        sym_list=train_dataset.get_sym_list(),
    )
    abc_loss = ABC_Loss(num_points_mesh=train_dataset.get_model_point_num())

    # gadd_loss = GADD_Loss(torch.device("cuda"))
    optimizer = optim.AdamW(
        estimator.parameters(), lr=opt.lr, weight_decay=opt.weight_decay
    )

    logger.log(f"=========Object 6D Pose Estimation Training Log========")
    logger.log(
        f"training start date and time: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
    )
    logger.log(f"data root: {opt.dataset_root}")
    logger.log(
        f"train set size: {len(train_dataset)}, test set size: {len(test_dataset)}"
    )
    ...
    logger.log(
        f"training on single obj: {opt.single_obj_test} exclude: {opt.exclude_single_obj}"
    )
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        logger.log(
            f"learning rate planning: start {lr_schedule[0]} end {lr_schedule[-1]}\n"
        )
        with tqdm(train_loader, unit="batch") as tepoch:
            for i, data in enumerate(tepoch):
                try:
                    ...
                    if data["obj_id"].item() == -1:
                        continue  # skip negative samples for training, or we can also include them with a special handling in the loss function (e.g., only use robust_loss for negative samples)
                except:
                    continue
                rgb, pt_cld, sample_2d, obj_id = (
                    data["img_crop"].cuda(),
                    data["point_cloud"].cuda(),
                    data["sample_2d"].cuda(),
                    data["obj_id"].cuda(),
                )
                target, model_points = (
                    data["sampled_model_pt_camera"].cuda(),
                    data["sampled_model_pt"].cuda(),
                )
                if epoch == 0 and i == 0:

                    logger.log("===== DEBUG MODEL SCALE =====")

                    logger.log(f"model_points shape: {model_points.shape}")

                    logger.log(
                        f"model_points min: {model_points.min().item()}, "
                        f"max: {model_points.max().item()}, "
                        f"mean: {model_points.mean().item()}"
                    )

                    logger.log(f"target shape: {target.shape}")

                    logger.log(
                        f"target min: {target.min().item()}, "
                        f"max: {target.max().item()}, "
                        f"mean: {target.mean().item()}"
                    )
                ...
                rx, tx, cx, _, _, robust_loss = estimator(
                    rgb, pt_cld, sample_2d, obj_id, recon_ref
                )

                if opt.loss == "adds":
                    loss, dist = adds_loss(
                        rx, tx, cx, target, model_points, obj_id, pt_cld, opt.w
                    )
                elif opt.loss == "abc":
                    loss, dist = abc_loss(
                        rx, tx, cx, target, model_points, pt_cld, opt.w
                    )
                else:
                    raise NotImplementedError

                trans_loss, trans_dist = translation_loss_fn(
                    tx,
                    pt_cld,
                    tar_t,
                )
                loss = loss + opt.trans_w * trans_loss + opt.recon_w * robust_loss
        ...
        if len(loss_list) > 0:
            logger.log(
                f"Epoch: {epoch}, Avg Loss: {sum(loss_list)/len(loss_list)}, Avg Recon Loss: {sum(robust_loss_list)/len(robust_loss_list)}"
            )
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                try:
                    ...
                    continue
                rgb = data["img_crop"].cuda()
                pt_cld = data["point_cloud"].cuda()
                sample_2d = data["sample_2d"].cuda()
                obj_id = data["obj_id"].cuda()
                target = data["sampled_model_pt_camera"].cuda()
                model_points = data["sampled_model_pt"].cuda()
                if opt.recon_choice == "model":
                    recon_ref = model_points
                elif opt.recon_choice == "depth":
                    recon_ref = pt_cld
        ...
        # save checkpoints
        last_path = os.path.join(cp_dir, "last.pth")
        if opt.save_interval > 0 and (epoch + 1) % opt.save_interval == 0:
            interval_path = os.path.join(cp_dir, f"epoch_{epoch+1:03d}.pth")
            torch.save(estimator.state_dict(), interval_path)
            logger.log(f"Saved checkpoint: {interval_path}")
        best_path = os.path.join(cp_dir, "best.pth")
        torch.save(estimator.state_dict(), last_path)
        ...
        if dist_value < best_test_dist:
            ...
            new_best = True
            saved = True
            torch.save(estimator.state_dict(), best_path)
        if dist_2cm > best_dist_2cm:
            logger.log(f"Reach best test performance, save checkpoint to {cp_dir}")
            best_dist_2cm = dist_2cm
            if not saved:
                torch.save(estimator.state_dict(), best_path)
        if dist_1cm > best_dist_1cm:
            logger.log(
                f"Reach best <1cm performance: "
                f"{best_dist_1cm:.2f}% -> {dist_1cm:.2f}%"
            )
            best_dist_1cm = dist_1cm
            torch.save(estimator.state_dict(), best_path)

        # update training settings
        if new_best and best_test_dist < opt.decay_margin:
            opt.lr *= opt.lr_rate
            opt.min_lr *= opt.lr_rate
            opt.w *= opt.w_rate
            opt.decay_margin *= opt.decay_rate
            logger.log(
                f"learning rate decay to {opt.lr}, minimum end learning rate decay to {opt.min_lr}, w decay to {opt.w}, decay_margin decay to {opt.decay_margin}"
            )
            optimizer = optim.AdamW(
                estimator.parameters(), lr=opt.lr, weight_decay=opt.weight_decay
            )
